...
public class DriveSubsystem extends SubsystemBase {
  ...
  public boolean printPath(Path path) {
		final Field[] k_fields = {
				new Field("x", 'f'),
        new Field("Y", 'f'),
        new Field("Left Velocity", 'f'),
		...
		for(int i=0; i<path.m_centerPath.length; i++) {
			writer.write(path.m_centerPath[i].x, path.m_centerPath[i].y, path.m_leftPath[i].velocity, path.m_rightPath[i].velocity);
		}
		writer.finish();
		return true;
  }
